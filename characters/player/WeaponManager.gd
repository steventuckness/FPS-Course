...
func init(_fire_point: Spatial, _bodies_to_exclude: Array):
	...
	for weapon in weapons:
		if weapon.has_method("init"):
			weapon.init(_fire_point, bodies_to_exclude)
			
	weapons[WEAPON_SLOTS.MACHINE_GUN].connect("fire", self, "alert_nearby_enemies")
	weapons[WEAPON_SLOTS.SHOTGUN].connect("fire", self, "alert_nearby_enemies")
